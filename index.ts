...
// Constants
const token: string = process.env.ACCESS_TOKEN || config.token || '';
...
bot.on("presenceUpdate", async (prev: Presence | null, now: Presence) => {
    try {
...
async function handlePresenceUpdate(previous_presence: Presence | null, current_presence: Presence) {
    let username: string;
    let nickname: string;
    ...
    console.log(chalk`{blueBright ${nickname}} (@${username}) is now playing {yellow ${current_presence.activities.map((activity) => activity.name)}} {grey (${current_presence.activities.length})} {grey [${guildname}]} `)

    var prev_activities: Activity[] = [];
    var prev_activity_names: string[] = [];
    ...
    var current_activities: Activity[] =
        config.ignoreUnverifiedActivities ? // Filter for verified activities, if desired.
            current_presence.activities.filter(act => act.applicationId != null)
            : current_presence.activities;
    var current_activity_names: string[] = current_activities.map(act => act.name);
 ...
 * @returns 
 */
async function pickRole(guild: Guild, name: string, is_assigning = true) {
    ...
    if (role == null) { // create role
        ...
        if (is_assigning) console.log(chalk`   {greenBright + role} {grey "${desiredRolename}"}`);
